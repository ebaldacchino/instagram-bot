...
const delay = (ms) => new Promise((resolve) => setTimeout(resolve, randomizeDelay(ms)));
const randomizeDelay = (ms) => ms + Math.random() * ms;
const getNewIndex = (indexes, images) => {
    let result = null;
    while (result == null) {
...
const instagram = {
    ...
    login: async (username, password) => {
        ...
        await delay(300);
        try {
            let buttonToLoginPage = await instagram.page.$('//a[contains(text(), "Log in")]');
    ...
    likeTagsProcess: async (tags = []) => {
        ...
        for (let tag of tags) {
            ...
            await delay(1000);
            const posts = await instagram.page.$$('main > article > div:nth-of-type(2) img');
    ...
    likeFollowersProcess: async (followers = []) => {
        for (let follower of followers) {
            ...
            try {
                const followers = await instagram.page.$$('div[aria-label="Followers"] li a[title]');
                for (let i = 0; i < followers.length; i++) {
                    ...
                    // change window
                    try {
                        const followersButton = await instagram.page.$(`a[href="/${follower}/followers/"]`);
                        if (!followersButton)
                            throw new Error();
                        ...
                        for (let i of indexes) {
                            ...
                            try {
                                ...
                                const likeButton = await instagram.page.$(`svg[aria-label="Like"][fill="#262626"]`);
                                if (likeButton)
